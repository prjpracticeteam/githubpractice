...
class intersect(pr_proc):
    ...
    def intersect(self):
        ...
        if self.list_1[1] and self.list_2[1] > 0:
            result_data = pr_proc.intersect(self.list_1[0] + self.list_2[0])
...
if __name__ == '__main__':
    try:
        ...
        if len(sys.argv) == 4:
            inter_pr = intersect
            res_len = inter_pr(sys.argv).intersect()
            print(f"Output: Process Name: Intersect, {sys.argv[1]}: {res_len[0]}, {sys.argv[2]}: {res_len[1]}, "
